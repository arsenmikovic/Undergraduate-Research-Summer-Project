...
"""
A vision transformer (ViT) model to classify the angle of a line drawn between two points in a small image.
Works with every pixel as a separate token (patch size = 1) with a transformer architecture.
...
class SinCos2DPositionalEncoding(nn.Module):
    ...
    """
    def __init__(self, num_patches: int, embed_dim: int, prior_embed_dim=4):
        super().__init__()
        self.num_patches = num_patches
        self.prior_embed_dim = prior_embed_dim
        self.embed_dim = embed_dim
        self.proj = nn.Linear(self.prior_embed_dim, self.embed_dim)
        ...
        self.register_buffer('pos_embed', self.build_sincos_embedding(coords, self.prior_embed_dim))
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=28, in_chans=1, prior_embed_dim=4):
        super().__init__()
        self.img_size = img_size
        self.patch_size = 1
        self.num_patches = (img_size // 1) ** 2
        self.patch_dim = in_chans
        self.prior_embed_dim = prior_embed_dim # Add this line
        self.proj = nn.Linear(self.patch_dim, self.prior_embed_dim)
        #self.act = nn.ReLU()  # <-- added nonlinearity
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=28, in_chans=1, embed_dim=16,
                 depth=1, num_heads=4, mlp_ratio=4.0, num_classes=8):
        super().__init__()
        self.patch_embed = PatchEmbed(img_size, in_chans, prior_embed_dim=4) # Pass embed_dim
...
def train(model, train_data, test_data, epochs=20000, lr=1e-4, weight_decay=0.3):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == 1 or epoch == epochs-1:
            model.eval()
            with torch.no_grad():
...
def generate_train_test_line_data(image_size=8, num_classes=16, train_ratio=0.8, test_ratio=1.0, outlier_prob=0.1, flip_num=0, blind=False):
    ...
    def make_data(pair_list):
        ...
        for A, B in pair_list:
            ...
            # Create a noisy/flipped version
            noisy_img = img.clone()
            flipped = False
            for _ in range(flip_num):
                if random.random() < outlier_prob:
                    flipped = True
                    # Choose a random point not on the line
                    flipped_point = random.choice([p for p in coords if p != A and p != B])
                    # Flip the pixel
                    noisy_img[0, flipped_point[1], flipped_point[0]] = 1.0 - noisy_img[0, flipped_point[1], flipped_point[0]]
            if flipped:
                images.append(noisy_img)
                labels.append(label)  # same label as original

        x = torch.stack(images)  # [2N, 1, H, W]
        y = torch.tensor(labels)
        return x, y

    train_pairs = pairs[:total]
    test_pairs = pairs[total:int(len(pairs) * test_ratio)]
    return make_data(train_pairs), make_data(test_pairs)
...
def robustness_score_exclude_AB_random(model, image_size=8, num_classes=16, num_samples=20):
    ...
    print(f"The robustness against random pixel flips (20 per line) is: {robustness_percentage:.2f}%")
