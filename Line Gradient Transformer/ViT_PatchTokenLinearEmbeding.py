...
A Vision Transformer (ViT) implementation with Linear patch embedding and positional encoding.
...
class Projected2DPositionalEncoding(nn.Module):
    def __init__(self, num_patches: int, embed_dim: int, learnable=True):
        super().__init__()
        self.num_patches = num_patches
        self.embed_dim = embed_dim
        grid_size = int(math.sqrt(num_patches))
        ...
        # Create normalized (i, j) coordinates in [0, 1]
        coords = torch.stack(torch.meshgrid(
            torch.arange(grid_size), torch.arange(grid_size), indexing='ij'
        ), dim=-1).float()  # shape: [grid_size, grid_size, 2]
        coords /= (grid_size - 1)  # normalize to [0, 1]
        ...
        # Learnable or fixed linear projection from (i, j) to embed_dim
    ...
    def forward(self, x):
        # x: [B, num_patches, embed_dim]
        pos_embed = self.proj(self.coords)  # [num_patches, embed_dim]
...
# --- I think we need to increase the order of complexity
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=28, patch_size=7, in_chans=1, embed_dim=16,
        ...
        #self.pos_embed = Projected2DPositionalEncoding(num_patches, embed_dim)
